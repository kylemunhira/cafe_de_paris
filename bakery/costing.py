...
def menu_addon_unit_cost(menu_addon) -> Decimal | None:
    recipes = Recipe.objects.filter(menu_addon=menu_addon).select_related("ingredient")
    if not recipes.exists():
        return None
    total = sum(
        recipe.quantity_required * recipe.ingredient.selling_price for recipe in recipes
...
def product_unit_costs() -> dict[int, Decimal]:
    ...
    recipes = Recipe.objects.filter(product__isnull=False).select_related("ingredient")
...
def menu_addon_unit_costs() -> dict[int, Decimal]:
    costs = defaultdict(Decimal)
    recipes = Recipe.objects.filter(menu_addon__isnull=False).select_related(
        "ingredient"
    )
    for recipe in recipes:
        costs[recipe.menu_addon_id] += (
            recipe.quantity_required * recipe.ingredient.selling_price
        )
    return {
        addon_id: cost.quantize(Decimal("0.01")) for addon_id, cost in costs.items()
    }
